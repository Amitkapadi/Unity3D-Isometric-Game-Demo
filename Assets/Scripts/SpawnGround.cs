	...
	void SpawnBuildings(){
		for (int i = 0; i < Size2LimitedBuildings.Length; i++) {
			if(PlayerPrefs.HasKey(Size2LimitedBuildings[i].name)){
				...
				content.transform.FindChild (Size2LimitedBuildings [i].name).GetComponent<Button> ().interactable = false;
				content.transform.FindChild (Size2LimitedBuildings [i].name).transform.GetChild(0).GetComponent<Text> ().text = "0";
